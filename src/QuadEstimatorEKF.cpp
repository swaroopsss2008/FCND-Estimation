...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	// update progress in position x based on velocity and delta time dt
	predictedState(0) = curState(0) + curState(3) * dt;
	// update progress in position y based on velocity and delta time dt
	predictedState(1) = curState(1) + curState(4) * dt;
	// update progress in position z based on velocity and delta time dt
	predictedState(2) = curState(2) + curState(5) * dt;

	//   attitude.Rotate_BtoI(<V3F>) to rotate a vector from body frame to inertial frame
	V3F accelI = attitude.Rotate_BtoI(accel);
	// update change in velocity  based on inertial frame accelaration and delta time dt
